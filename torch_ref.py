...
def transcribe(model_dir, wav_path):
    ...
    print(f"Audio: {len(audio_array)} samples ({audio_seconds:.1f}s)", file=sys.stderr)
    ...
    prefill_toks_per_s = (prompt_len / t_prefill) if t_prefill > 0 else 0.0
    decode_toks_per_s = (len(generated) / t_decode) if t_decode > 0 else 0.0
    ...
    print(f"Model load:            {t_model_load:.3f} s", file=sys.stderr)
    print(f"Feature extraction:    {t_feature:.3f} s", file=sys.stderr)
    print(f"Encoder:               {t_encoder:.3f} s", file=sys.stderr)
    print(f"Prefill:               {t_prefill:.3f} s ({prefill_toks_per_s:.2f} tok/s)", file=sys.stderr)
    print(f"Decode:                {t_decode:.3f} s", file=sys.stderr)
    print(f"Detokenize:            {t_detok:.3f} s", file=sys.stderr)
    print(f"Decode throughput:     {decode_toks_per_s:.2f} tok/s", file=sys.stderr)
    print(f"End-to-end:            {t_total:.3f} s", file=sys.stderr)
    print(f"End-to-end RTF:        {end_to_end_rtf:.3f}x", file=sys.stderr)
